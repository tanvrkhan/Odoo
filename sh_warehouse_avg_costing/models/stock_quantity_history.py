...
class StockQuantityHistory(models.TransientModel):
    ...
    def open_at_date(self):
        ...
        if active_model == 'stock.valuation.layer':
            ...
            action['domain'] = [
                ('create_date', '<=', self.inventory_datetime), ('product_id.type', '=', 'product')]
            if self.warehouse_id:
                action['domain'] = [('create_date', '<=', self.inventory_datetime), (
                    'product_id.type', '=', 'product'), ('warehouse_id', '=', self.warehouse_id.id)]
            action['display_name'] = format_datetime(
                self.env, self.inventory_datetime)
            return action
