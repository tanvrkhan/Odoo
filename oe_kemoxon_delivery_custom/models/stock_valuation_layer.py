...
class StockValuationLayer(models.Model):
                ...
                if record.stock_move_id:
                    ...
                    if record.stock_move_id.quantity_done == 0:
                        ...
                        earliest_valuation = self.env['stock.valuation.layer'].search(
                            [('stock_move_id', '=', record.stock_move_id.id)], order='id asc', limit=1)
                        cost_of_earliest_valuation = earliest_valuation.unit_cost
                        for valuation in all_value_layers:
                            if valuation.product_id==record.product_id:
                                if valuation != earliest_valuation:
                                    valuation.unit_cost = cost_of_earliest_valuation
                                    ...
                                    amount_to_post = valuation.unit_cost * valuation.quantity
                                    accounting_header.state = 'draft'
                                    if accounting_header.has_reconciled_entries:
                                        break
                                    else:
                                        ...
                                        for ae in accounting_entries:
                                            if ae.amount_currency != 0:
                                                if ae.amount_currency > 0:
                                                    ae.with_context(check_move_validity=False).amount_currency = valuation.unit_cost * valuation.quantity
                                                    ae.with_context(check_move_validity=False).debit = valuation.unit_cost * valuation.quantity
                                                elif ae.amount_currency < 0:
                                                    ae.with_context(check_move_validity=False).amount_currency = -1 * valuation.unit_cost * valuation.quantity
    ...
    def calculate_costing_from_later_dates(self,record):
        all_valuations_after = self.env['stock.valuation.layer'].search(['&','&',('product_id','=',record.product_id.id),('stock_move_id','!=',record.stock_move_id.id),('stock_move_id.date','>',record.stock_move_id.date)])
        remaining_quantity = record.quantity
        accumulated_quantity = 0
        amount2 = 0
        quantity2 = record.quantity
        for later_valuation in all_valuations_after:
            if(later_valuation.quantity>0):
                remaining_quantity+= later_valuation.quantity
                if (remaining_quantity <= 0):
                    quantity2 += later_valuation.quantity
                    amount2 += later_valuation.value
                    accumulated_quantity += later_valuation.quantity
                    if(remaining_quantity==0):
                        break
                else:
                    quantity2 = (later_valuation.quantity - remaining_quantity)
                    amount2 += quantity2 * later_valuation.unit_cost
                    break
        record.unit_cost = amount2 / (record.quantity * -1)
        record.value = record.quantity * record.unit_cost
        record.account_move_id.state = 'draft'
