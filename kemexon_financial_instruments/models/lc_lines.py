...
class LcLines(models.Model):
    ...
    type = fields.Selection(related='fi_lc_ids.type', string='Type', store=True)
    lc_type = fields.Selection(related='fi_lc_ids.lc_type', string='LC Type', store=True)
    sub_limit = fields.Selection(related='fi_lc_ids.sub_limit', string='Sub Limit', store=True)
    issuance_date = fields.Date(related='fi_lc_ids.issuance_date', string='Issuance Date', store=True)
    expiry_date = fields.Date(related='fi_lc_ids.expiry_date', string='Expiry Date', store=True)
    is_active = fields.Boolean(related='fi_lc_ids.is_active', string='Is Active', store=True)
