...
class InvoiceControllerBI(models.Model):
    ...
    def check_existing_invoice(self, invoicenumber):
        existing_invoice = self.env['account.move'].search(
            [('fusion_invoice_number', '=', str(invoicenumber))])
        if existing_invoice:
                ...
                if rec.invoicestatus == 'Active':
                    if rec.invoiceopenstatus == 'Send To Accounting':
                        ...
                        if buysell == "Buy":
                            ...
                            if existing_invoice:
                                lock_date = existing_invoice.company_id.fiscalyear_lock_date
                                if not lock_date or existing_invoice.date > lock_date:
                                                ...
                                                if cfline['costtype'] != 'VAT':
                                                    ...
                                                    for product_lines in product_linez:
                                                        if cfline['costtype'] == 'Primary Settlement':
                                                            ...
                                                            'quantity'],2)) * -1 == float(round(product_lines.quantity,2))) \
                                                                    and (cfline[
                                                            ...
                                                            'quantity'],2)) == 0 or float(round(product_lines.quantity,2))==1))
                                                                                                    and (cfline['price'] == product_lines.price_unit or (cfline['extendedamount'] == product_lines.price_unit or cfline['extendedamount'] == product_lines.price_unit*-1)))):
                                                                self.update_existing_line(product_lines, company, cfline,
                                                                                          extended_multiplier,
                                                                                          cashflow_lines_all, rec.invoicenumber,
                                                                                          showExceptionsUI)
                                                        ...
                                                        #     'material'] == product_lines.product_id.name) or (
                                                        #               product_lines.product_id.name == "Down payment" and cfline[
                                                        #           'costtype'] in down_payment_types) and cfline[
                                                        ...
                                                        #     'material'] == product_lines.product_id.name) or (
                                                        #               product_lines.product_id.name == "Down payment" and cfline[
                                                        #           'costtype'] in down_payment_types)
                                                        #       and (float(round(cfline['extendedamount'], 2)) == round(
                                ...
                                if not lock_date or existing_invoice.date > lock_date:
                                    ...
                                    if existing_invoice.line_ids:
                                        ...
                                        if receivables > payables :
                                            if existing_invoice.move_type != 'out_invoice':
                                                ...
                                                if cfline['costtype']!='VAT':
                                                    ...
                                                    for product_lines in product_linez:
                                                        if cfline['costtype'] == 'Primary Settlement':
                                                            ...
                                                            # downpayment check
                                                            if (
                                                                    product_lines.product_id.name == "Down payment" or product_lines.product_id.name ==
                                                                    cfline[
                                                                        'material']) and cfline[
                                                                'costtype'] in down_payment_types and (((
                                                                    float(round(cfline[
                                                                ...
                                                                'quantity'],2)) * -1 == float(round(product_lines.quantity,2)) or ((cfline[
                                                                                                                    'quantity'] == 0 or product_lines.quantity == 1)))
                                                                                                        ...
                                                                                                        and (cfline[
                                                                                                                 'extendedamount'] == product_lines.price_unit or (
                                                                                                                     cfline[
                                                                                                                         'extendedamount'] == product_lines.price_unit or
                                                                                                                     cfline[
                                                                                                                         'extendedamount'] == product_lines.price_unit * -1)))):
                                                                self.update_existing_si_line(product_lines, company, cfline,extended_multiplier, rec.invoicenumber,showExceptionsUI)
                                                            # handling other costs
                                    ...
                                    log_error = self.env['fusion.sync.history.errors'].log_error(
                                        'InvoiceControllerBI',
                                        rec.invoicenumber,
                                        'Invoice of locked period',
                                        rec.internalcompany)
                                    if showExceptionsUI:
                                        raise UserError('Invoice of locked period')
                            else:
                                log_error = self.env['fusion.sync.history.errors'].log_error(
                                    'InvoiceControllerBI',
                                    rec.invoicenumber,
    ...
    def update_existing_line(self, existing_line, company, cf,extended_multiplier, cashflow_lines_all, invoicenumber,showExceptionsUI):
        ...
        if cf['costtype'] == 'Primary Settlement':
            ...
            if pol:
                ...
                existing_line.product_id = product.id
                existing_line.product_uom_id = uom.id
                existing_line.quantity = float(cf['quantity']) if (float(cf['extendedamount']) / float(
                    cf['quantity']) * extended_multiplier) >= 0 else float(cf['quantity']) * -1
    ...
    def update_existing_si_line(self, existing_line, company, cf, extended_multiplier, invoicenumber,showExceptionsUI):
        ...
        if cf['costtype'] == 'Primary Settlement':
            ...
            if sol:
                ...
                existing_line.product_uom_id = uom.id
                existing_line.quantity = float(cf['quantity']) if (float(cf['extendedamount'])/ float(cf['quantity'])*extended_multiplier) >= 0 else float(cf['quantity']) *-1
                existing_line.price_unit = float(cf['extendedamount'])/ float(cf['quantity']) if float(cf['extendedamount']) >0 else float(cf['extendedamount'])*-1/ float(cf['quantity'])
