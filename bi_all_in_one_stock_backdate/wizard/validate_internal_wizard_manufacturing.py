...
class WizardManufacturingTransfer(models.TransientModel):
            ...
            if self._context.get('mrp_id'):
                ...
                for mrp in custom_mrp_prod:
                    ...
                    if mrp.product_id.categ_id.property_valuation == 'real_time':
                        ...
                        for stock_move in custom_mrp_prod.move_raw_ids:
                            for account_move in stock_move.account_move_ids:
                                self.env.cr.execute("UPDATE account_move SET date = %s, remarks=%s WHERE id = %s",
                                    [finished_move.date, finished_move.move_remark, account_move.id]) 
                        for account_move in finished_move.account_move_ids:
                            self.env.cr.execute("UPDATE account_move SET date = %s, remarks=%s WHERE id = %s",
                                    [raw_move.date, raw_move.move_remark, account_move.id]) 
    ...
    def skip_backdate(self):
        ...
        if active_models == 'stock.picking':
            ...
            if self._context.get('mrp_id'):
                custom_mrp_prod = self.env['mrp.production'].browse(self._context.get('mrp_id'))
                custom_mrp_prod.with_context(Skip=True).button_mark_done()
