...
class MassAssignBackdate(models.Model):
    ...
    def on_click_confirm(self):
        if self.invoice_date.date() >= datetime.now().date():
        ...
        for pay_id in self.env['res.config.settings'].sudo().search([],order="id desc", limit=1):
            if pay_id.enable_backdate:
                for account_move in self.account_move_ids:
                    self.env.cr.execute("UPDATE account_move SET invoice_date = %s, remarks=%s WHERE id = %s",
                    [self.invoice_date, self.remarks, account_move.id])
